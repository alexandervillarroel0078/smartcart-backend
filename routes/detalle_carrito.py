...
def actualizar_producto_detalle(id_detalle):
    ...
    if exito:
        registrar_bitacora(request.usuario['id'], f"Actualizó cantidad del producto detalle ID {id_detalle} a {nueva_cantidad}", request.remote_addr)
...
def eliminar_producto_detalle(id_detalle):
    ...
    if exito:
        registrar_bitacora(request.usuario['id'], f"Eliminó un producto del detalle del carrito (ID {id_detalle})", request.remote_addr)
