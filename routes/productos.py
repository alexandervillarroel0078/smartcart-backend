...
def agregar_producto():
    if request.usuario['rol'] != 'almacenero':
    ...
    # ✅ Aquí debes pasar también umbral_stock
    exito = registrar_producto(nombre, descripcion, precio, stock, umbral_stock, id_categoria, imagen)
    if exito:
        registrar_bitacora(
...
def obtener_producto(id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT p.id, p.nombre, p.descripcion, p.precio, p.stock, p.umbral_stock, p.id_categoria
            FROM productos p
            WHERE p.id = %s
        """, (id,))
...
def actualizar_stock(id):
    ...
    exito = sumar_stock_producto(id, int(cantidad))
